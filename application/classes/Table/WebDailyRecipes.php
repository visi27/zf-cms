...
class Table_WebDailyRecipes extends Table_Abstract implements Interface_iForm
{

    protected $_name = 'web_daily_recipes';
    ...
    public function createNew(Zend_Form $formObj)
    {
        ...
        $recipe_id = explode("-", $formObj->getElement('recipe_id')->getValue());
        $row->recipe_id = trim($recipe_id[0]);
        ...
        $this->moveOthers($row->order_nr, $publish_date);
    ...
    public function selectRowsForGrid($id = array(), $sortField = null, $sortDir = null)
    {
        ...
        $select->setIntegrityCheck(false)
            ...
            ->join("receipts", "$this->_name.recipe_id = receipts.id", "");
    ...
    public function selectRowForGrid(array $args)
    {
        ...
        $select->setIntegrityCheck(false)
            ->from($this, array(
            "$this->_name.id",
            "CONCAT(receipts.id, ' - ', receipts.title) as recipe_id",
    ...
    public function moveOthers($order_nr, $publish_date)
    {
        $data = array(
            'order_nr' => new Zend_Db_Expr('order_nr+1')
        );
        $order_where = $this->getAdapter()->quoteInto('order_nr >= ?', $order_nr);
        $date_where = $this->getAdapter()->quoteInto('publish_date = ?', $publish_date);
