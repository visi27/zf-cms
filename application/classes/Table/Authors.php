...
class Table_Authors extends Table_Abstract implements Interface_iForm
{
            ...
            foreach ($filters as $field => $filter) {
                if (count($filter) > 0) {
                    foreach ($filter as $operator => $value)
                        $select->orWhere($field . $operator . '?', $value);
                }
            }
        }
    ...
    public function selectRowsForSearch($filters = array(), $sortField = null, $sortDir = null)
    {
        ...
        if (null != $sortField) {
            ...
            $select->order("$this->_name.firstname");
        }
    ...
    public function createNew(Zend_Form $formObj)
    {
        ...
        $row->firstname = $formObj->getElement('firstname')->getValue();
        
        $row->lastname = $formObj->getElement('lastname')->getValue();
