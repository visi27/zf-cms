...
class Table_WebSponsored extends Table_Abstract implements Interface_iForm
{

    protected $_name = 'web_sponsored';
    ...
    public function createNew(Zend_Form $formObj)
    {
        ...
        $section = explode("-", $formObj->getElement('section_id')->getValue());
        $row->section_id = trim($section[0]);
    ...
    public function selectRowsForGrid($filters = array(), $sortField = null, $sortDir = null)
    {
        ...
        $select_articles->setIntegrityCheck(false)
            ...
            "start_date",
            "end_date",
            "blog_sections.title as section_id"
        ))
            ->join("blog_articles", "$this->_name.element_id = blog_articles.id", "")
            ->join("blog_sections", "$this->_name.section_id = blog_sections.id", "")
        ...
        if (count($filters) > 0) {
            foreach ($filters as $field => $filter) {
                if (count($filter) > 0) {
                    foreach ($filter as $operator => $value)
                        $select_articles->where($field . $operator . '?', $value);
                }
            }
        }
        ...
        if (count($filters) > 0) {
            foreach ($filters as $field => $filter) {
                if (count($filter) > 0) {
                    foreach ($filter as $operator => $value)
                        $select_receipts->where($field . $operator . '?', $value);
                }
            }
        }
    ...
    public function selectRowForGrid(array $args)
    {
        ...
        if ($element->type == 1) {
            ...
            $select->setIntegrityCheck(false)
                ...
                "CONCAT(blog_sections.id, ' - ', blog_sections.title) as section_id"
            ))
                ->join("blog_articles", "$this->_name.element_id = blog_articles.id", "")
                ->join("blog_sections", "$this->_name.section_id = blog_sections.id", "")
