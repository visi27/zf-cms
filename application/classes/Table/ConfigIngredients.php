...
class Table_ConfigIngredients extends Table_Abstract implements Interface_iForm
{

    protected $_name = 'config_ingredients';
    ...
    public function createNew(Zend_Form $formObj)
    {
        ...
        $row->default_unit = $formObj->getElement('default_unit')->getValue();
        
        $category = explode("-", $formObj->getElement('category')->getValue());
        $row->category = trim($category[0]);
        
        $row->notes = $formObj->getElement('notes')->getValue();
    ...
    public function selectRowsForGrid($id = array(), $sortField = null, $sortDir = null)
    {
        ...
        $select->setIntegrityCheck(false)
            ...
            "$this->_name.id",
            "$this->_name.name",
            "$this->_name.default_unit",
            "config_ingredients_category.name as category"
        ))
            ->join("config_ingredients_category", "$this->_name.category = config_ingredients_category.id", "");
    ...
    public function selectRowForGrid(array $args)
    {
        ...
        $select->setIntegrityCheck(false)
            ...
            "CONCAT($this->_name.category,' - ', config_ingredients_category.name) as category",
            "$this->_name.default_unit",
            "notes"
        ))
            ->join("config_ingredients_category", "$this->_name.category = config_ingredients_category.id", "")
