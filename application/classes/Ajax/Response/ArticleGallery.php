<?php
class Ajax_Response_ArticleGallery{
	...
	public function getGalleryHtml($filter = array()){
		...
		$imagesDir = 'files/articles/' . $articleId.'/medium';
		...
		$html.='</div>
				...
				<div id="dialog-confirm"  style="display:none" title="Konfirmo Vendosjen si Imazh Profili!">
  					<p><span class="ui-icon ui-icon-alert" style="float: left; margin: 0 7px 20px 0;"></span>Ky imazh do te jete imazhi i profilit per kete artikull. Jeni te sigurte?</p>
				</div>
				';
	        					...
	        					'Vendose Foto Horizontale Profili': function() {
	        						...
	        						$.ajax({
										...
										data: {
											...
											parameter: 'imgFile:'+image_file+';articleId:".$articleId."'
										},
										context: this,
	...
	public function assignProfileImage($filter = array()){
		...
		$article = new Table_BlogArticles();
		
		$output = $article->assignProfileImage($articleId, $fileName);
	...
	public function assignVerticalProfileImage($filter = array()){
	    ...
	    $output = $article->assignVerticalProfileImage($articleId, $fileName);
	    echo json_encode($output);
	}
