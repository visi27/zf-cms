...
class Utility_Grid {
	...
	public function __construct($moduleId, $action="default", $elements=array(), $ajax = false,
		...
		$this->setScript('			
			...
			jQuery("#'. $this->dialogId . '").remove();
				
			// wrap the form element inside a div element
			//jQuery("#'. $this->zendFormId . '").wrap(\'<div id="' . $this->dialogId . '"/>\');
			//Test BOOTSTRAP MODAL
			jQuery("#'. $this->zendFormId . '").wrap(\'<div id="' . $this->dialogId . '" class="modal fade"></div>\');
			jQuery("#'. $this->zendFormId . '").wrap(\'<div class="modal-dialog"></div>\');
			jQuery("#'. $this->zendFormId . '").wrap(\'<div class="modal-content"></div>\');
			
			jQuery(\'<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">Confirmation</h4></div>\').insertBefore("#'. $this->zendFormId . '");
			jQuery(\'<div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">Mbyll</button></div>\').insertAfter("#'. $this->zendFormId . '");
			
			jQuery("#'. $this->zendFormId . '").wrap(\'<div class="modal-body"></div>\');
			...
			//	hide: "slide",
			//	modal: true,
			//	beforeClose: function(event, ui) { jQuery("ul.ui-autocomplete").hide(); }
			//});
				
			//make the form visible
			jQuery("#'. $this->zendFormId . '").css("display", "block");
			
			// dont allow to wrap names	
			jQuery("table#data-table td").attr("nowrap","nowrap");
				
			jQuery(".errors li").css("marginLeft","0px");
			jQuery(".elementWrap").css("marginBottom","8px");
		...
		if ($this->getAction() == "submit_form"){
			...
			$displayFormDialog = '
				jQuery(function() {
			         ...
			         jQuery( "#' . $this->getDialogId() . '" ).find(".modal-title").html( "'.$this->zendFormObj->getElement("form_mode")->getValue().'" );

			         //show the ZendForm in a Bootstrap Modal
					 jQuery("#'.$this->getDialogId().'").modal("show");		
	...
	public function setDialogSize($width="600px", $height="auto", $dialogId = null ) {
		
		$dialogId = is_null($dialogId)?$this->getDialogId():$dialogId;

		$this->setScript ( 'jQuery( "#' . $dialogId . ' > .modal-dialog" ).css( "width", "' . $width . '" );' );
		$this->setScript ( 'jQuery( "#' . $dialogId . ' > .modal-dialog" ).css( "height", "' . $width . '" );' );
	...
	*/
	...
	/**
	 * This function makes possible, printing the entire grid rowset.
	 * It serializes the grid object and Post-s it to the TcPdf Printer. 
	 */
	public function exportDataToPdf(){
	/*
		 // get en instance of the zend registry
		$registry = Zend_Registry::getInstance();
		
		// get from the registry the maximum number of allowed rows to be printed
		$maxAllowedRecords = $registry->get('config')->max_records_to_pdf;
		
		// check if the filters are required to be printed
		if($filterData){
		...
		$pfdExport.=' // onClick function
				...
				var maxAllowedRows = '.$registry->get('config')->max_records_to_pdf.';
						
		jQuery("#pdfExport").click(function(){
				
			if(jQuery(".gridPannel #totalRows").html().trim() > maxAllowedRows){	
						
				if(confirm("Are you sure to print more than "+maxAllowedRows+" rows ?")){
					printToPdf = true;
				}
			...
			if(printToPdf==true){	
				...
				// create a temporarly form, in order to POST the data
				jQuery("<form id=\"pdf-form\" method=\"post\" action=\"index.php?c=gridToPdf\"><input type=\"hidden\" name=\"gridObjId\" value=\"'.$this->viewPaneId.'\"></form>").appendTo(".page-content-wrapper .page-content");
				jQuery("<input type=\"hidden\" name=\"headerText\" value=\"'.$headerText.'\">").appendTo("#pdf-form");
				jQuery("<input type=\"hidden\" name=\"toolbarText\" value=\"'.preg_replace("/[\n\r]/","",$appliedFilters).'\">").appendTo("#pdf-form");
				jQuery("<input type=\"hidden\" name=\"content\" value=\"\">").appendTo("#pdf-form");
	...
	*/		
	...
	private function createButtonNew(){	
		...
		$buttonNewScript.='	
			...
			//Set Bootstrap Modal Title
			jQuery( "#' . $this->getDialogId() . '" ).find(".modal-title").html( "Shto Rekord Te Ri" );
			...
			if(selectedRowId){
    			...
    			jQuery.ajax({
    					...
    					$buttonEditScript.= "\r\n".'
    			         ...
    			         //Set Bootstrap Modal Title
    			         jQuery( "#' . $this->getDialogId() . '" ).find(".modal-title").html( "Modifiko Rekordin" );
